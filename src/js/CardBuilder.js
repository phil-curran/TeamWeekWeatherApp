...
export default class CardBuilder {
  static async buildCard(dayData, weekData) {
    ...
    console.log(IconPicker.findIcon(dayData.weather[0].description));
    ...
    newDayDiv.innerHTML += `<img class="weatherIcon img-fluid" width="100" height="100" src="${icon}" />`;
    ...
    for (let i = 0; i < weekData.list.length; i++) {
      ...
      if (weekData.list[i].dt_txt.includes(time)) {
        ...
        newDiv.innerHTML += `<img class="weatherIcon img-fluid" src='${icon}' />`;
