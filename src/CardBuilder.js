...
export default class CardBuilder {
  static async buildCard(dayData, weekData) {
    ...
    modal.innerHTML =
    '<div class="modal-dialog" role="document">' + '<div class="modal-content">' + '<div class="modal-header">' + '<h5 class="modal-title" id="myModal">' + "Weather Details" + '</h5>' + '<button type="button" class="btn-close" data-bs-dismiss="modal"></button>' + '</div>' + '<div class="modal-body" id="modal-body">' + '<p>'+ "Cloud Coverage: " + `${dayData.clouds.all}%` + '</p>' + '<p>'+ "Description: " + `${dayData.weather[0].description}` + '</p>' + '<p>'+ "Humidity " + `${dayData.main.humidity}%` + '</p>' + '</div>' + '</div>' + '</div>';
    ...
    for (let i = 0; i < weekData.list.length; i++) {
      ...
      if (weekData.list[i].dt_txt.includes(time)) {
        ...
        modal.innerHTML =
        '<div class="modal-dialog" role="document">' + '<div class="modal-content">' + '<div class="modal-header">' + '<h5 class="modal-title" id="myModal">' + "Weather Details" + '</h5>' + '<button type="button" class="btn-close" data-bs-dismiss="modal"></button>' + '</div>' + '<div class="modal-body" id="modal-body">' + '<p>'+ "Cloud Coverage: " + `${weekData.list[i].clouds.all}%` + '</p>' + '<p>'+ "Description: " + `${weekData.list[i].weather[0].description}` + '</p>' + '<p>'+ "Precipitation: " + `${weekData.list[i].pop}%` + '</p>' + '</div>' + '</div>' + '</div>';
